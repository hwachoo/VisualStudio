//array/parctice2/js
let data = 
`[{"id":1,"first_name":"Ina","last_name":"Snarr","email":"isnarr0@deviantart.com","gender":"Female","ip_address":"172.196.159.13"},
...
let objAry = JSON.parse(data);//문자열을 object타입으로 변환

console.log(objAry);

let result = objAry.reduce(function(accum, ele, idx) {
  if(idx == 0) {
    accum += '<ul>'
  }
  accum += `<li>id: ${ele['id']}, first_name: ${ele['first_name']} </li>`;
  if(idx == objAry.length -1) {
    accum += '</ul>';
  }
  return accum; //다음순번의 초기값(계속 누적하여 tag 생성)
}, '');
console.log(result);
